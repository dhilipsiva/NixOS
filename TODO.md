...
**GATE 6 (FULLY PASSED 2026-07-07):** the install rehearsal is green end-to-end (disko + LUKS + closure + GRUB + sops), sandboxed, **no real hardware touched**. **Human visual sign-off DONE** — the owner ran a graphics-enabled VM and confirmed Hyprland + waybar + the home-manager dotfiles render (software GL; true RTX behaviour is Phase 7). The visual check also caught a pre-existing dotfile bug — `.config/hypr/hyprland.conf` used the dwindle-only `togglesplit`/`pseudo` binds under `layout = master`; both are now commented. **CAVEAT:** sops-green proves the MECHANISM (fake `vm-test.yaml` + throwaway key), NOT that the real host key decrypts `secrets.yaml` — real login is a Phase 7 enrollment step. **This is the go/no-go for real hardware — now GREEN, subject to the Phase 7 human items (real by-id, host-key enrollment, key/password rotation).**
